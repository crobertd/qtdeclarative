...
void QQmlIncubator::forceCompletion()
{
    ...
    while (Loading == status()) {
        while (Loading == status() && !d->waitingFor.isEmpty()) {
            QQmlIncubatorPrivate::QIPBase *current =  d->waitingFor.first();
            QQmlIncubatorPrivate *currentIncubator = static_cast<QQmlIncubatorPrivate *>(current);
            currentIncubator->incubate(i);

            while ((currentIncubator->status == Loading) && (currentIncubator->waitingFor.isEmpty() == false)) {
                current = currentIncubator->waitingFor.first();
                currentIncubator = static_cast<QQmlIncubatorPrivate *>(current);
                currentIncubator->incubate(i);
            }
        }
        if (Loading == status())
